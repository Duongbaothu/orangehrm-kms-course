...
if [[ $param = "all" ]]; then
    ...
    node node_modules/@cucumber/cucumber/bin/cucumber-js --parallel 4 -f json:report/$param/chrome.json $featureFile --tags "not @setup"
else 
    export BROWSER=$param
    node node_modules/@cucumber/cucumber/bin/cucumber-js --parallel 4 -f json:report/$param/cucumber_report.json $featureFile --tags "not @setup"
fi
result=$?
export endTime=$(date)
node multiReport.js
